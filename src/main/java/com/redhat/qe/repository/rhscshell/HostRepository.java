...
public class HostRepository extends Repository<Host> implements IHostRepository, IHostRepositoryExtended{
	...
	public Host create(Host host){
		return Host.fromResponse(getShell().sendAndCollect(createCommand(host)).unexpect("error:"));
	}
	...
	private IResponse _show(String nameOrId){
		return getShell().sendAndCollect(String.format("show host \"%s\"", nameOrId));
	}
	...
	private IResponse _deactivate(String nameOrId) {
		return getShell().sendAndCollect(String.format("action host %s deactivate", nameOrId));
	}
	...
	public IResponse activate(String nameOrId){
		return getShell().sendAndCollect(String.format("action host %s activate", nameOrId)).expect("status-state: complete");
	}
	...
	public IResponse _destroy(Host host, String options) {
		...
		return getShell().sendAndCollect(command.toString());
	}
	...
	public IResponse _list(String options){
		return (options == null) ? getShell().sendAndCollect("list hosts") : getShell().sendAndCollect("list hosts" + " " + options);
	}
	...
	private IResponse _update(Host entity) {
		...
		return getShell().sendAndCollect(command.toString()).unexpect("error");
	}

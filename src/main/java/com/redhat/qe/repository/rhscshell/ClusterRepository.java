...
public class ClusterRepository extends Repository<Cluster>implements IClusterRepository {
	...
	public Cluster create(Cluster cluster){
		return Cluster.fromResponse(getShell().sendAndCollect(createCommand(cluster)).unexpect("error:"));
	}
	...
	private IResponse _show(String nameorId){
		return getShell().sendAndCollect(String.format("show cluster %s",nameorId));
	}
	...
	public IResponse _destroy(String nameOrId){
		 return getShell().sendAndCollect(String.format("remove cluster %s", nameOrId));
	}
	...
	private IResponse _update( Cluster entity) {
		return getShell().sendAndCollect(String.format("update cluster %s --name %s", entity.getId(), entity.getName()));
	}
	...
	public IResponse _list(String options){
		...
		return ( options == null) ? getShell().sendAndCollect(command) :	getShell().sendAndCollect(command +" " + options);
	}
